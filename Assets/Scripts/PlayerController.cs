...
public class BasicFPCC : MonoBehaviour
{
    ...
    [HideInInspector] public bool inputKeyLeftClick = false;  // is key Pressed
    ...
    void ProcessInputs()
    {
        if (useLocalInputs)
        {
            ...
            inputKeyLeftClick = Input.GetKeyDown(keyLeftClick);
        }
        ...
        if (inputKeyLeftClick)
        {
            TriggerDestroyCube();
        }
    }

    private void TriggerDestroyCube()
    {
        GameObject objectHit = GetClickedGameObject();
        if (objectHit != null)
        {
            Destroy(objectHit);
        }
    ...
    private GameObject GetClickedGameObject()
    {
        ...
        if (Physics.Raycast(ray, out hit, playerRange, castingMask))
        {
